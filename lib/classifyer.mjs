...
const classifyItem = async (item, modelName = "Gemini 3.1 Flash Lite") => {
    // 1. Load config
...
- type: One of 'green_bin', 'black_bin', 'blue_bin', 'garbage', 'others', 'error'
- reason: A short explanation of why it belongs to this category.
Ensure the 'type' is strictly one of the allowed values and in lowercase. If you are unable to classify the item because of the poor image quality, messy objects, set the type to 'error' and explain the reason in the 'reason' field.`;

    let userContent;

    if (isImage) {
        userContent = [
            { type: "text", text: `Classify this item.${instructionText}` },
            { type: "image_url", image_url: { url: item } }
        ];
    } else {
        userContent = `Classify this item, focus on the main portion of the photo, ignore the background and the surrounding objects: ${item}.${instructionText}`;
    }
